init -990 python in mas_submod_utils:
    #Since Windows is the only system which gets away with this, we should enforce the depdency
    __awc_deps = {"Better Loading": (None, None)} if not renpy.windows else dict()
    ...
    awc_submod = Submod(
        ...
        dependencies=__awc_deps
    )

init -989 in awc_utils:
    import store
